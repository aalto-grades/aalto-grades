...
describe(
  ...
  () => {
    ...
    it('should get correct CSV template when attainments exist (teacher in charge)', async () => {
    ...
    it('should respond with 404 not found if the assessment model has no attainments', async () => {
      res = await request
...
`);
    ...
    it('should export only unexported grades if override is false', async () => {
      res = await request
        .get(
          '/v1/courses/6/assessment-models/52/grades/csv/sisu' +
...
`);
    ...
    it('should filter CSV grades based on instance ID if URL query included', async () => {
      res = await request
        .get(
...
`);
    ...
    it('should filter CSV grades based on student numbers if URL query included', async () => {
      res = await request
        .get(
          '/v1/courses/9/assessment-models/42/grades/csv/sisu' +
...
`);
    ...
    it('should return results for all instaces connected to the assessment model if no filters', async () => {
      res = await request
        .get(
          '/v1/courses/9/assessment-models/42/grades/csv/sisu?' +
...
`);
    ...
    it('should respond with 400 bad request, if (optional) completionLanguage param is not valid', async () => {
      res = await request
        .get(
          '/v1/courses/1/assessment-models/1/grades/csv/sisu?completionLanguage=xy'
        )
        .set('Cookie', cookies.adminCookie);
    ...
    it('should respond with 400 bad request, if (optional) assessmentDate param is not valid date', async () => {
      res = await request
        .get(
          '/v1/courses/1/assessment-models/1/grades/csv/sisu?assessmentDate=notValidDate'
        )
        .set('Cookie', cookies.adminCookie);
    ...
    it('should respond with 404 not found, if assessment model does not exist', async () => {
      ...
      checkErrorRes(
        [`assessment model with ID ${badId} not found`],
        HttpCode.NotFound
      );
    });
...
describe(
  ...
  () => {
    ...
    function checkStudentNumbers(
      finalGrades: Array<FinalGrade>,
      studentNumbers: Array<string>
    ): void {
      expect(
        finalGrades
          .map((finalGrade: FinalGrade) => finalGrade.studentNumber)
          .sort()
    ...
    it('should get the appropriate number of grade options depending on how many grades exist', async () => {
      res = await request
      ...
      function checkGradesOfStudent(
        ...
        const gradesOfStudent:
          | Array<{grade: number; status: Status}>
          | undefined = res.body.data
          .find((finalGrade: FinalGrade) => {
      ...
      checkGradesOfStudent('344625', [{grade: 4, status: Status.Pass}]);
    });

    it('should filter returned grades based on student number if URL query included', async () => {
      res = await request
        .get(
          '/v1/courses/9/assessment-models/41/grades' +
    ...
    it('should filter returned grades based on instance ID if URL query included', async () => {
      res = await request
    ...
    it(
      ...
      async () => {
        res = await request
          .get(
            '/v1/courses/9/assessment-models/42/grades?instanceId=26' +
    ...
    it('should not filter returned grades if no filters included in URL query', async () => {
      res = await request
    ...
    it('should respond with 400 bad request, if course or instance ID not is valid', async () => {
      res = await request
  ...
  () => {
    it('should process CSV succesfully when attainments and users exist (admin user)', async () => {
      const csvData: fs.ReadStream = fs.createReadStream(
        path.resolve(__dirname, '../mock-data/csv/grades.csv'),
        'utf8'
      );
      res = await request
    ...
    it('should process CSV succesfully when attainments and users exist (teacher in charge)', async () => {
      jest.spyOn(TeacherInCharge, 'findOne').mockResolvedValueOnce(mockTeacher);
    ...
    it('should create users when a user does not exist in database', async () => {
      ...
      const csvData: fs.ReadStream = fs.createReadStream(
        path.resolve(
          __dirname,
          '../mock-data/csv/grades_non-existing_students.csv'
        ),
        'utf8'
      );
      res = await request
    ...
    it('should mark attainment grades as manual', async () => {
      const csvData: fs.ReadStream = fs.createReadStream(
        path.resolve(__dirname, '../mock-data/csv/grades_one.csv'),
        'utf8'
      );
    ...
    it('should mark correct grader ID', async () => {
      const csvData: fs.ReadStream = fs.createReadStream(
        path.resolve(__dirname, '../mock-data/csv/grades_one_2.csv'),
        'utf8'
      );
    ...
    it('should determine whether an attainment was passed based on min required grade', async () => {
      const csvData: fs.ReadStream = fs.createReadStream(
        path.resolve(__dirname, '../mock-data/csv/grades_failing.csv'),
        'utf8'
      );
    ...
    it('should allow uploading multiple grades to the same attainment for a student', async () => {
        ...
        await request
          ...
          .attach(
            ...
            fs.createReadStream(
              path.resolve(
                __dirname,
                `../mock-data/csv/grades_multiple_uploads_${n}.csv`
              ),
              'utf8'
            ),
      ...
      expect(
        grades.find((val: AttainmentGrade) => val.grade === 1)
      ).toBeDefined();
      expect(
        grades.find((val: AttainmentGrade) => val.grade === 5)
      ).toBeDefined();
    });

    it('should process big CSV succesfully (1100 x 178 = 195 800 individual attainment grades)', async () => {
      const csvData: fs.ReadStream = fs.createReadStream(
        path.resolve(__dirname, '../mock-data/csv/grades_big.csv'),
        'utf8'
      );
      res = await request
    ...
    it('should mark completion and expiry date if provided in the request body', async () => {
      ...
      await Promise.all(
        [28, 47, 86, 127, 138, 172].map(async (id: number): Promise<void> => {
    ...
    it('should respond with 400 bad request, if the CSV has only student numbers, no grading data', async () => {
      const invalidCsvData: fs.ReadStream = fs.createReadStream(
        path.resolve(
          __dirname,
          '../mock-data/csv/grades_only_student_numbers.csv'
        ),
        'utf8'
      );
      res = await request
    ...
    it('should respond with 400 bad request, if the CSV has grades with incorrect type', async () => {
      const invalidCsvData: fs.ReadStream = fs.createReadStream(
        path.resolve(__dirname, '../mock-data/csv/grades_incorrect_type.csv'),
        'utf8'
      );
      res = await request
        .post('/v1/courses/1/assessment-models/1/grades/csv')
    ...
    it(
      ...
      async () => {
        const invalidCsvData: fs.ReadStream = fs.createReadStream(
          path.resolve(__dirname, '../mock-data/csv/grades_over_max.csv'),
          'utf8'
        );
        ...
        function error(row: number, grade: number): string {
          return (
            `CSV file row ${row} column 2 uploaded grade "${grade}" is larger` +
            ' than maximum allowed grade "5"'
          );
        }

        checkErrorRes(
          [error(3, 5.01), error(4, 6), error(5, 1000000)],
          HttpCode.BadRequest
        );
      }
    );

    it('should respond with 400 bad request, if the CSV file header parsing fails', async () => {
      const invalidCsvData: fs.ReadStream = fs.createReadStream(
        path.resolve(
          __dirname,
          '../mock-data/csv/grades_incorrect_header_columns.csv'
        ),
        'utf8'
      );
      res = await request
      ...
      function errorMessage(
        ...
        return (
          `Header attainment data parsing failed at column ${column}. ` +
    ...
    it('should respond with 400 bad request, if the CSV file grading data parsing fails', async () => {
      const invalidCsvData: fs.ReadStream = fs.createReadStream(
        path.resolve(
          __dirname,
          '../mock-data/csv/grades_incorrect_grade_rows.csv'
        ),
        'utf8'
      );
      res = await request
    ...
    it('should respond with 400 bad request, if the CSV file parsing fails (one row invalid length)', async () => {
      const invalidCsvData: fs.ReadStream = fs.createReadStream(
        path.resolve(__dirname, '../mock-data/csv/grades_invalid_row.csv'),
        'utf8'
      );
      res = await request
        .post('/v1/courses/1/assessment-models/1/grades/csv')
      ...
      checkErrorRes(
        ['Invalid Record Length: expect 7, got 6 on line 4'],
        HttpCode.BadRequest
      );
    });
    ...
    it('should respond with 400 bad request, if the file content-type not text/csv', async () => {
      const csvData: fs.ReadStream = fs.createReadStream(
        path.resolve(__dirname, '../mock-data/csv/grades.csv'),
        'utf8'
      );
    ...
    it('should respond with 400 bad request, if the file extension incorrect (.txt)', async () => {
      const txtFile: fs.ReadStream = fs.createReadStream(
        path.resolve(__dirname, '../mock-data/csv/wrong_file_type.txt'),
        'utf8'
      );
      res = await request
        .post('/v1/courses/1/assessment-models/1/grades/csv')
    ...
    it('should respond with 400 bad request, if the CSV file not found in the request.', async () => {
      res = await request
        .post('/v1/courses/1/assessment-models/1/grades/csv')
    ...
    it('should respond with 400 bad request, if validation fails (non-number assessment model id)', async () => {
      const csvData: fs.ReadStream = fs.createReadStream(
        path.resolve(__dirname, '../mock-data/csv/grades.csv'),
        'utf8'
      );
...
describe('Test POST /v1/courses/:courseId/assessment-models/:assessmentModelId/grades/calculate', () => {
  async function checkGraderId(
    result: AttainmentGrade,
    cookie: Array<string>
  ): Promise<void> {
    const selfInfo: supertest.Response = await request
      .get('/v1/auth/self-info')
  ...
  async function checkGrade(
    ...
    if (checkGrader) checkGraderId(result as AttainmentGrade, cookie);
  }
  ...
  it('should calculate multiple correct grades based on instance ID and student numbers', async () => {
    checkSuccessRes(
      await request
  ...
  it('should respond with 404 not found if instance does not have any students assigned', async () => {
    res = await request
      .post('/v1/courses/9/assessment-models/42/grades/calculate')
      .send({
        instanceId: 28,
    ...
    expect(res.body.data).not.toBeDefined();
    expect(res.body.errors[0]).toBe(
      'No student numbers found from instance ID 28'
    );
  });
});
...
describe(
  ...
  () => {
    ...
    it('should respond with 400 bad request, if float grade send when attainment expects integer', async () => {
      res = await request
    ...
    it('should respond with 404 not found, if grade does not exist', async () => {
      ...
      checkErrorRes(
        [`attainment grade with ID ${badId} not found`],
        HttpCode.NotFound
      );
    });
