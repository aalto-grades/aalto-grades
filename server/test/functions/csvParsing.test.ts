...
describe('Test CSV header parser', () => {
  it('should parse correctly formatted header of attainment CSV file', async () => {
    let result: Array<Attainment> = await parseHeaderFromCsv(
      ['StudentNumber', 'name1', 'name5', 'name9', 'name16'],
      1
    );
    expect(result.length).toBe(4);
    expect(result.map((attainment: Attainment) => attainment.id)).toEqual(
      expect.arrayContaining([1, 5, 9, 16])
    );

    result = await parseHeaderFromCsv(
      ['STUDENTNUMBER', 'name2', 'name6', 'name10'],
      2
    );
    ...
    result = await parseHeaderFromCsv(['studentnumber', 'name1'], 1);
  ...
  it('should throw error if parsing fails due to invalid header column', async () => {
    ...
    try {
      ...
      checkError(error, HttpCode.BadRequest, [errorMessage(3, 'name2', 1)]);
    }
    ...
    try {
      ...
      checkError(error, HttpCode.BadRequest, [errorMessage(4, 'fake-name', 1)]);
    }
  ...
  it('should parse correctly formatted grades in the CSV file', () => {
    ...
    result.forEach((student: StudentGrades, index: number) => {
      ...
      student.grades.forEach(
        (grade: AttainmentGradeModelData, index: number) => {
