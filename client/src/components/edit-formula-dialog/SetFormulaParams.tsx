...
export default function SetFormulaParams(props: {
  ...
  if (!props.params && !props.childParams) {
    if (
      props.attainment.formula === props.formula.id &&
      props.attainment.formulaParams
    ) {
      const oldParams: ParamsObject = structuredClone(
        props.attainment.formulaParams
      ) as ParamsObject;
  ...
  function handleParamChange(
    ...
    if (props.params) props.params[param] = Number(event.target.value);
  }
  ...
  return (
    <>
      ...
      >
        {props.attainment.subAttainments?.map((attainment: AttainmentData) => {
