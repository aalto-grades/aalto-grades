...
export default function FileLoadDialog(props: {
  ...
  const [expiryDate, setExpiryDate]: State<string | undefined> = useState<
    string | undefined
  >(undefined);
  ...
  const [fileName, setFileName]: State<string | null> = useState<string | null>(
    null
  );
  const [validationError, setValidationError]: State<string> =
    useState<string>('');
  const [fileErrors, setFileErrors]: State<Array<string>> = useState<
    Array<string>
  >([]);
      ...
      uploadGradeCsv.mutate(
        ...
        },
        {
          onSuccess: () => {
            snackPack.push({
              msg:
                'File processed successfully, grades imported.' +
        ...
        <DialogContent sx={{pb: 0}}>
          ...
          >
            ...
            <Box
              ...
              alt={
                'A picture of a sheet with three columns. First one is student ' +
                ...
                <Container sx={{overflowX: 'scroll'}}>
                  <Box sx={{my: 1}}>
                    <Typography variant="body2" sx={{mb: 3}}>
                      Note. Manually setting an expiry date for the grades will
                      override the &apos;days valid&apos; value of the grades.
                    </Typography>
                    <TextField
          ...
          {fileErrors.length !== 0 && (
            <>
              <Typography id={'file_content_errors'} sx={{mt: 2}}>
                {errorInstructions}
              </Typography>
